...
export default function MorningRitual() {
  ...
  return (
    <View className="p-6 rounded-3xl bg-white border border-gray-100 shadow-sm overflow-hidden">
      <View className="flex-row items-center justify-between mb-6">
        <View>
          <Text className="text-xl font-bold text-gray-900">Morning Ritual</Text>
          <Text className="text-sm text-gray-500">Start your day with intention.</Text>
        </View>
        {isRitualComplete && (
          <View className="px-3 py-1 bg-green-100 rounded-full">
            <Text className="text-green-700 text-xs font-bold uppercase">Complete</Text>
          </View>
        )}
      </View>

      <View className="mb-8">
        <Text className="text-sm font-bold text-gray-700 mb-3">Morning Mood</Text>
      ...
      <View className="gap-4">
        {ritualSteps.map((step) => {
          ...
          return (
            ...
            >
              <View className={`p-2 rounded-xl ${isDone ? 'bg-purple-100' : 'bg-gray-100'}`}>
                <Icon size={20} color={isDone ? '#9333EA' : '#6B7280'} />
              </View>
              <View className="flex-1">
                <Text className={`font-bold text-sm ${isDone ? 'text-purple-900' : 'text-gray-900'}`}>
                  {step.label}
                </Text>
